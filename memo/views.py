...
def index():
    # メモ全件取得
    memos = Memo.query.filter_by(user_id=current_user.id).all() # <=リスト13.2変更
    # 画面遷移
...
def create():
    ...
    if form.validate_on_submit():
        ...
        # 登録処理
        memo = Memo(title=title, content=content, user_id=current_user.id) # <= 変更
        db.session.add(memo)
        db.session.commit()
        # フラッシュメッセージ
        flash("登録しました")
...
def update(memo_id):
    ...
    # 見つからない場合は404エラーを表示
    target_data = Memo.query.filter_by(id=memo_id, user_id=current_user.id).first_or_404() # <= 変更
    # Formに入れ替え
...
# 削除
@memo_bp.route("/memo/delete/<int:memo_id>")
@login_required
def delete(memo_id):
    # データベースからmemo_idに一致するメモを取得し、
    # 見つからない場合は404エラーを表示
    memo = Memo.query.filter_by(id=memo_id, user_id=current_user.id).first_or_404() # <= 変更
    # 削除処理
